...
Here, $tt_i$ is a vector of times where we estimate the value of $y$, and $yy_i$ is the vector of estimates.
...
\lstinputlisting[style=mcode, caption={A function implementing Euler's method}, label={lst:euler_method}]{../code/chap09/euler.m}
...
After computing the growth rate, we add an element both \lstinline{tt} and \lstinline{yy}, incrasing their length by one.  Then, when the loop exits, we plot \lstinline{yy} as a function of \lstinline{tt}, adding labels and units as good engineering practice.
...
\includepdf[pages=-,frame,pagecommand={},width=\textwidth]{../code/chap09/euler_compare.pdf}
...
\lstinputlisting[style=mcode]{../code/chap09/rat_growth.m}
